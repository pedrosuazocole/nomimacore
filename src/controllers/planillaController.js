...
const PlanillaController = {
    ...
    guardarDetalle(req, res) {
        ...
        try {
            ...
            // Notificacion de WhatsApp (fire-and-forget): no se espera la
            // respuesta ni se deja que un fallo de red retrase o rompa el
            // guardado de la planilla, que ya quedo hecho arriba.
            const planilla = PlanillaModel.obtener(planillaId);
            enviarWhatsApp(
                `✅ Planilla procesada en NominaCore HN\n\n` +
                `${planilla.nombre}\n` +
                `Periodo: ${planilla.fecha_inicio} al ${planilla.fecha_fin}\n` +
                `Total a pagar: L. ${Number(planilla.total_pagar).toLocaleString('es-HN', { minimumFractionDigits: 2 })}`
            ).catch(() => {});
