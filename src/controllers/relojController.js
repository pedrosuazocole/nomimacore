...
const RelojController = {
    ...
    async marcar(req, res) {
        try {
            const { empleado_id, tipo, lat, lng } = req.body;
            ...
            const resultado = tipo === 'salida'
                ? TurnoModel.marcarSalida(empleado_id, nombreArchivo, coords)
                : TurnoModel.marcarEntrada(empleado_id, nombreArchivo, coords);
