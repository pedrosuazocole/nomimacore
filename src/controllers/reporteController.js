...
// ANTES esto era siempre "el mes calendario actual", lo cual causaba
// que reportes se vieran "vacios"/"rotos" si una planilla tenia una
// fecha fuera del mes en curso (ej. capturada con el ano/mes
// equivocado por error de digitacion). Ahora se detecta el rango REAL
// donde existen planillas (desde la mas antigua hasta la mas
// reciente), y solo se usa el mes actual si todavia no hay ninguna
// planilla en el sistema.
function rangoPorDefecto() {
    const rangoReal = db.prepare(`SELECT MIN(fecha_inicio) AS min, MAX(fecha_fin) AS max FROM planillas`).get();
    if (rangoReal && rangoReal.min && rangoReal.max) {
        return { inicio: rangoReal.min, fin: rangoReal.max };
    }
    const hoy = new Date();
...
const ReporteController = {
    ...
    asistencia(req, res) {
        ...
        const filas = db.prepare(`
            SELECT t.*, e.nombre_completo, e.departamento, emp.radio_metros AS radio_permitido_metros
        ...
        // Estado de ubicacion por fila, usando el radio permitido REAL de
        // la empresa de cada empleado (no un numero fijo para todos).
        filas.forEach(f => {
            const radio = f.radio_permitido_metros || 150;
            f.entradaFueraDeRango = f.distancia_entrada_metros != null ? f.distancia_entrada_metros > radio : null;
            f.salidaFueraDeRango = f.distancia_salida_metros != null ? f.distancia_salida_metros > radio : null;
        });
        ...
        const totales = {
            ...
            fueraDeRango: filas.filter(f => f.entradaFueraDeRango || f.salidaFueraDeRango).length
        };
        ...
        if (req.query.export === 'xlsx') {
            return enviarExcel(res, 'reporte-asistencia.xlsx', {
                ...
                headers: ['Empleado', 'Departamento', 'Fecha', 'Hora Entrada', 'Distancia Entrada (m)', 'Hora Salida', 'Distancia Salida (m)', 'Horas Trabajadas', 'Tiene Foto Entrada', 'Tiene Foto Salida'],
                filas: filas.map(r => [
                    r.nombre_completo, r.departamento, r.fecha,
                    r.hora_entrada_real || '—', r.distancia_entrada_metros != null ? r.distancia_entrada_metros : '—',
                    r.hora_salida_real || '—', r.distancia_salida_metros != null ? r.distancia_salida_metros : '—',
