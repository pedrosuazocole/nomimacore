...
function migrarColumnasFaltantes(db) {
    ...
    agregarColumnaSiFalta(db, 'configuracion', 'whatsapp_apikey_callmebot', "TEXT");
    agregarColumnaSiFalta(db, 'configuracion', 'hikvision_api_key', "TEXT");
    agregarColumnaSiFalta(db, 'empleados', 'codigo_hikvision', "TEXT");

    // Genera una API key aleatoria automaticamente la primera vez (si
    // queda vacia), para que el puente de Hikvision tenga algo que usar
    // sin que el usuario tenga que inventarsela a mano).
    if (tablaExiste(db, 'configuracion')) {
        const cfg = db.prepare('SELECT hikvision_api_key FROM configuracion WHERE id = 1').get();
        if (cfg && !cfg.hikvision_api_key) {
            const crypto = require('crypto');
            const nuevaKey = crypto.randomBytes(24).toString('hex');
            db.prepare('UPDATE configuracion SET hikvision_api_key = ? WHERE id = 1').run(nuevaKey);
            console.log('🔑 Se genero automaticamente una API key para la integracion con Hikvision (visible en Configuracion).');
        }
    }
    ...
    // Geolocalizacion: coordenadas de referencia de cada empresa, y
    // coordenadas + distancia calculada de cada marca del Reloj de
    // Asistencia (para saber si el empleado marco fuera del negocio).
    agregarColumnaSiFalta(db, 'empresas', 'lat', "REAL");
    agregarColumnaSiFalta(db, 'empresas', 'lng', "REAL");
    agregarColumnaSiFalta(db, 'empresas', 'radio_metros', "INTEGER DEFAULT 150");
    agregarColumnaSiFalta(db, 'turnos_horarios', 'lat_entrada', "REAL");
    agregarColumnaSiFalta(db, 'turnos_horarios', 'lng_entrada', "REAL");
    agregarColumnaSiFalta(db, 'turnos_horarios', 'distancia_entrada_metros', "REAL");
    agregarColumnaSiFalta(db, 'turnos_horarios', 'lat_salida', "REAL");
    agregarColumnaSiFalta(db, 'turnos_horarios', 'lng_salida', "REAL");
    agregarColumnaSiFalta(db, 'turnos_horarios', 'distancia_salida_metros', "REAL");
