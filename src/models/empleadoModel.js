...
const EmpleadoModel = {
    ...
    crear(data) {
        const stmt = db.prepare(`
            ...
            (codigo_contable, codigo_hikvision, nombre_completo, departamento, cargo, empresa_id, empresa, cuenta_contable,
             salario_base, tipo_pago, tipo_jornada, fecha_ingreso, estado)
            VALUES (@codigo_contable, @codigo_hikvision, @nombre_completo, @departamento, @cargo, @empresa_id, @empresa, @cuenta_contable,
                    @salario_base, @tipo_pago, @tipo_jornada, @fecha_ingreso, @estado)
        `);
    ...
    actualizar(id, data) {
        const stmt = db.prepare(`
            UPDATE empleados SET
                ...
                codigo_hikvision = @codigo_hikvision,
    ...
    obtenerPorCodigoHikvision(codigoHikvision) {
        return db.prepare('SELECT * FROM empleados WHERE codigo_hikvision = ? AND estado = ?').get(String(codigoHikvision), 'ACTIVO');
    },
...
function normalize(data) {
    ...
    return {
        ...
        codigo_hikvision: data.codigo_hikvision || null,
