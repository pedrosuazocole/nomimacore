...
const { distanciaMetros } = require('../services/geoService');
...
const TurnoModel = {
    ...
    marcarEntrada(empleadoId, fotoNombreArchivo = null, coords = null, marcaExplicita = null) {
        const fecha = marcaExplicita ? marcaExplicita.fecha : fechaHondurasHoy();
        const hora = marcaExplicita ? marcaExplicita.hora : horaHondurasAhora();
        ...
        const { lat, lng, distancia, dentroDelRango } = calcularUbicacion(empleadoId, coords);

        db.prepare(`
            INSERT INTO turnos_horarios (empleado_id, fecha, hora_entrada_real, foto_entrada, lat_entrada, lng_entrada, distancia_entrada_metros, tipo_turno, es_dia_libre)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'DIARIO', 0)
            ON CONFLICT(empleado_id, fecha) DO UPDATE SET
                hora_entrada_real = excluded.hora_entrada_real,
                foto_entrada = excluded.foto_entrada,
                lat_entrada = excluded.lat_entrada,
                lng_entrada = excluded.lng_entrada,
                distancia_entrada_metros = excluded.distancia_entrada_metros,
                es_dia_libre = 0,
                updated_at = datetime('now','localtime')
        `).run(empleadoId, fecha, hora, fotoNombreArchivo, lat, lng, distancia);

        const turno = db.prepare('SELECT * FROM turnos_horarios WHERE empleado_id = ? AND fecha = ?').get(empleadoId, fecha);
        const avisoUbicacion = dentroDelRango === false ? ` ⚠️ Marcaste a ${distancia}m del negocio.` : '';
        return { ok: true, mensaje: `Entrada marcada a las ${hora}.${avisoUbicacion}`, turno };
    },

    marcarSalida(empleadoId, fotoNombreArchivo = null, coords = null, marcaExplicita = null) {
        const fecha = marcaExplicita ? marcaExplicita.fecha : fechaHondurasHoy();
        const hora = marcaExplicita ? marcaExplicita.hora : horaHondurasAhora();
        const existente = db.prepare('SELECT * FROM turnos_horarios WHERE empleado_id = ? AND fecha = ?').get(empleadoId, fecha);
        ...
        db.prepare(`
            UPDATE turnos_horarios SET
                hora_salida_real = ?, horas_trabajadas = ?, foto_salida = ?,
                lat_salida = ?, lng_salida = ?, distancia_salida_metros = ?,
                updated_at = datetime('now','localtime')
            WHERE empleado_id = ? AND fecha = ?
        `).run(hora, horas, fotoNombreArchivo, lat, lng, distancia, empleadoId, fecha);

        const turno = db.prepare('SELECT * FROM turnos_horarios WHERE empleado_id = ? AND fecha = ?').get(empleadoId, fecha);
        const avisoUbicacion = dentroDelRango === false ? ` ⚠️ Marcaste a ${distancia}m del negocio.` : '';
        return { ok: true, mensaje: `Salida marcada a las ${hora}. Trabajaste ${horas} h hoy.${avisoUbicacion}`, turno };
    },

    // Dice si hoy ya hay entrada/salida marcada para un empleado — lo usa
    // la integracion de Hikvision para decidir si un evento del reloj
    // biometrico es una entrada o una salida (el dispositivo no siempre
    // lo distingue el mismo).
    obtenerEstadoHoy(empleadoId, fecha) {
        return db.prepare('SELECT * FROM turnos_horarios WHERE empleado_id = ? AND fecha = ?').get(empleadoId, fecha);
    },
...
// Dado un empleado y las coordenadas capturadas al marcar, busca la
// ubicacion de referencia de SU empresa y calcula que tan lejos marco.
// Si el empleado no dio permiso de ubicacion (coords=null) o su empresa
// todavia no tiene coordenadas configuradas, simplemente no se puede
// evaluar — se guarda lo que haya, sin bloquear el marcaje por esto.
function calcularUbicacion(empleadoId, coords) {
    if (!coords || coords.lat == null || coords.lng == null) {
        return { lat: null, lng: null, distancia: null, dentroDelRango: null };
    }

    const empleado = db.prepare('SELECT empresa_id FROM empleados WHERE id = ?').get(empleadoId);
    const empresa = empleado && empleado.empresa_id
        ? db.prepare('SELECT lat, lng, radio_metros FROM empresas WHERE id = ?').get(empleado.empresa_id)
        : null;

    const distancia = (empresa && empresa.lat != null && empresa.lng != null)
        ? distanciaMetros(coords.lat, coords.lng, empresa.lat, empresa.lng)
        : null;
    const dentro = distancia != null ? distancia <= (empresa.radio_metros || 150) : null;

    return { lat: coords.lat, lng: coords.lng, distancia, dentroDelRango: dentro };
}
