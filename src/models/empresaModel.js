...
const EmpresaModel = {
    ...
    crear(data) {
        const info = db.prepare(`
            INSERT INTO empresas (nombre, rtn, direccion, telefono, lat, lng, radio_metros, estado)
            VALUES (@nombre, @rtn, @direccion, @telefono, @lat, @lng, @radio_metros, @estado)
        `).run(normalize(data));
    ...
    actualizar(id, data) {
        ...
        db.prepare(`
            UPDATE empresas SET
                ...
                telefono = @telefono, lat = @lat, lng = @lng, radio_metros = @radio_metros,
                estado = @estado,
...
function normalize(data) {
    return {
        ...
        radio_metros: data.radio_metros ? Number(data.radio_metros) : 150,
